...
def auto_kategori(teks, tipe):
    ...
    if tipe == "KELUAR":
        peta = {
            "makan":      ["makan","minum","kopi","snack","sarapan","warung","resto","indomaret","alfamart","jajan"],
...
async def cmd_excelbulan(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    args = ctx.args
    if not args:
        await update.message.reply_text("Format: `/excelbulan YYYY-MM`", parse_mode="Markdown")
        return
    bulan = args[0].strip()
