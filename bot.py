...
Fitur:
- Catat pengeluaran & pemasukan (bisnis + pribadi)
- Catat produksi harian
- Laporan harian otomatis tiap malam
- Rekap produksi harian & bulanan
- Export Excel 4 sheet + grafik
...
def ensure_sheets(sh):
    ...
    make("Transaksi", ["ID","Tanggal","Waktu","Tipe","Kategori","Deskripsi","Nominal","Sumber","Pencatat"], "1E7A4A")
...
def auto_kategori(teks, tipe):
    ...
    if tipe == "KELUAR":
        peta = {
            "makan":      ["makan","minum","kopi","snack","sarapan","warung","resto"],
            "transport":  ["bensin","grab","gojek","ojek","parkir","tol","bus"],
...
async def cmd_menu(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    teks = (
        ...
        "`1.5jt` = Rp 1.500.000\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "_Laporan otomatis dikirim tiap malam_"
    )
...
async def cmd_labarugi(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    teks += "━━━━━━━━━━━━━━━━━━━━━\n"
    emoji = "🟢" if laba_bersih >= 0 else "🔴"
    teks += f"{emoji} *LABA BERSIH : {fmt(laba_bersih)}*"
...
def build_excel(tx_records, prod_records, label):
    ...
    for row in ws1.iter_rows(min_row=5, max_row=7):
    ...
    hdrs2 = ["No","Tanggal","Waktu","Tipe","Kategori","Deskripsi","Nominal","Pencatat","Metode"]
    ws2.append(hdrs2)
    ...
    for i, r in enumerate(tx_records, 1):
        ws2.append([
            i, r["Tanggal"], r["Waktu"], r["Tipe"],
            r.get("Kategori",""), r["Deskripsi"],
            int(r["Nominal"]), r.get("Pencatat",""), r.get("Sumber","")
        ])
        rn = ws2.max_row
    ...
    for i, r in enumerate(prod_records, 1):
        ...
        style_row(ws3, rn, len(hdrs3))
        prod_map[r["Produk"]] = prod_map.get(r["Produk"], 0) + int(r["Jumlah"])
    auto_width(ws3)

    # ── Sheet 4: Rekap Produksi + Grafik ──
...
async def cmd_excelbulan(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    args = ctx.args
    if not args:
        await update.message.reply_text("Format: `/excelbulan YYYY-MM`\nContoh: `/excelbulan 2025-01`", parse_mode="Markdown")
        return
    bulan = args[0].strip()
    if not re.match(r"^\d{4}-\d{2}$", bulan):
        await update.message.reply_text("Format salah. Gunakan: `YYYY-MM`", parse_mode="Markdown")
        return
...
async def kirim_laporan_harian(ctx: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        teks  = f"🌙 *LAPORAN HARIAN OTOMATIS*\n"
        teks += f"📅 {tgl}\n"
        teks += "━━━━━━━━━━━━━━━━━━━━━\n\n"
        teks += "💰 *KEUANGAN*\n"
        ...
        if prod_map:
            ...
            teks += "🏭 *PRODUKSI*: _Tidak ada data_\n"

        teks += "\n━━━━━━━━━━━━━━━━━━━━━\n"
        teks += "_Laporan otomatis_ 🤖"

        await ctx.bot.send_message(chat_id=OWNER_ID, text=teks, parse_mode="Markdown")
    except Exception as e:
...
async def handle_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if lower.startswith("produksi") or lower.startswith("prod "):
        ...
        balasan += f"\n🕐 {wkt} | 📅 {tgl}"
        await update.message.reply_text(balasan, parse_mode="Markdown")
        return
    ...
    if not hasil:
        await update.message.reply_text(
            "Format tidak dikenali. Ketik /menu untuk bantuan.",
            parse_mode="Markdown"
        )
        return
...
def main():
    ...
    # Jadwal laporan harian otomatis
